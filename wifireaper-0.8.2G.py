...
if __name__ == "__main__":
    while True:
        ...
        print('[' + T + '*' + W + ']' "Choose the Wi-Fi want to encrypted\n")
        ...
        os.system('airodump-ng -c ' + channel_num  + ' ' + wlancard_name.lower().strip())
        ...
        while True:
            encrypt = raw_input("Input the AP encryption mode : (None \ WEP \ WPA \ WPA2)")
        ...
        while True:
            ...
            print ('[' + T + '*' + W + ']' "Frame\t\t\tTime\t\t\t\thttp\t\t\tsrc\t\tdst\n")
            os.system("cat "+filename+".txt")

            input = raw_input('[' + G + '?' + W + ']' "Capture over! Would you want to capture contiue? [y/n]")
            if input.lower().strip() == "n":
                break
        input = raw_input('[' + G + '?' + W + ']' "Capture over! Would you want to run the script again? [y/n]")
        if input.lower().strip() == "n":
            sys.exit()     
